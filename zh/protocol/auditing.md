# 資安審計

## Scallop的Sui智能合約受過資安審計與開源嗎?

Scallop 已經完成了 Zellic、OtterSec 和 MoveBit 的智能合約審計。Scallop Lend 是一個開源協議，運行在 Sui 區塊鏈上，因此每個人都可以成為 Scallop Lend 的審計員。

### 核心智能合約 (借貸)

**Scallop 智能合約審計報告 by Zellic:**

{% file src="../.gitbook/assets/Scallop - Zellic Audit Report.pdf" %}

**Scallop 智能合約審計報告  by OtterSec:**

{% file src="../.gitbook/assets/Scallop_Audit.pdf" %}

{% file src="../.gitbook/assets/scallop_audit_2024.pdf" %}

**Scallop 智能合約審計報告  by MoveBit:**

{% file src="../.gitbook/assets/Scallop Smart Contract Audit Report.pdf" %}

{% file src="../.gitbook/assets/Scallop Audit Report-2025-11-24.pdf" %}

**Scallop 智能合約審計報告  by Asmptotic:**

{% file src="../.gitbook/assets/scallop-audit-2025-12-02.pdf" %}

**Scallop sui-lending-protocol** �&#xDD17;**:**\
\
[https://github.com/scallop-io/sui-lending-protocol](https://github.com/scallop-io/sui-lending-protocol)

### sCoin 封包

{% file src="../.gitbook/assets/Scallop -  SCoin Audit Report-2025-6-20.pdf" %}
audited by MoveBit
{% endfile %}

### veSCA

{% file src="../.gitbook/assets/Scallop VeSCA Audit Report.pdf" %}
audited by MoveBit
{% endfile %}

{% file src="../.gitbook/assets/Scallop VeSCA Audit Report-2025-6-20.pdf" %}
audited by MoveBit
{% endfile %}

### SCA (Scallop 代幣)

{% file src="../.gitbook/assets/Scallop Token Audit Report.pdf" %}
audited by MoveBit
{% endfile %}

### 借款激勵

{% file src="../.gitbook/assets/Scallop -  Borrow Incentive Audit Report-2025-6-20.pdf" %}
audited by MoveBit
{% endfile %}
